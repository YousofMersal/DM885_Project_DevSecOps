...
describe('Solver service unit test', () => {
  ...
  test('Getting newly inserted item from database"', async () => {
    ...
    expect(JSON.stringify(dbResult)).toMatch(
      new RegExp(
        '{"name":"newEntry","image":"linkminizinc","solver_id":[0-9]+}'
      )
    )
  })
  ...
  test('Getting newly changed item from database', async () => {
    ...
    expect(JSON.stringify(dbResult)).toMatch(
      new RegExp(
        '{"name":"NewNameHere","image":"NewImageHere","solver_id":[0-9]+}'
      )
    )
  })
  ...
  test('Deleting item from database', async () => {
    const dbResult = await outerDeleteSolver(2, db, '4')
    expect(dbResult).not.toBeNull()
  })

  test('Testing that deleted item does not exist in database', async () => {
    const dbResult = await outerGetByName(2, db, '4')
    expect(dbResult).toBeNull()
  })
})
